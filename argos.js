...
(async () => {
	...
	[serverHTTP, serverWS] = startServers(serverHTTP, serverWS);
...
async function setUpApp() {
	const tray = new Tray(path.join(__dirname, 'public/img/icon/network-96.png'));
...
async function createWindow() {
	...
	mainWindow.loadURL(`file://${__dirname}/views/main.ejs`);
...
function connectToWebServer(retry = false) {	
	...
	if ((!webServer.connected && webServer.active && webServer.attempts < 3) || (retry && !webServer.connected)) {
		...
		const webSocket = new WebSocket('wss://'+webServer.address);
